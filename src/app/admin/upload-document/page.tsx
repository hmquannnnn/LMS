...
const IMAGE_PLACEHOLDER =
  "https://placehold.co/600x400/png?text=%E1%BA%A2nh\nThumbnail&font=arial";
...
const Index = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(
        `https://notion-api.splitbee.io/v1/page/${inputValue}`,
      );
  ...
  const onPostDocument = async (document: DocumentInput, file: File | null) => {
    ...
    if (file) {
      ...
      const isGetImageSuccess = await blobUrlToFile(firstImageUrl)
        .then((file) => {
          formData.append("thumbnail", file);
          return true;
        })
        .catch((error) => {
          ...
          setIsLoadingSubmit(false);
          alert(
            "Lưu ảnh từ link thất bại. Vui lòng thay đổi link ảnh hoặc tải ảnh lên.",
          );
  ...
  const handleCheckImageSaveAbility = async () => {
    ...
    await blobUrlToFile(firstImageUrl)
      .then((file) => {
        setFileInput(file);
      ...
      .catch((error) => {
        ...
        setIsLoadingCheckImage(false);
        alert(
          "Link ảnh không cho phép lưu do lỗi bảo mật của trang chứa ảnh.\n Vui lòng thay đổi link ảnh hoặc tải ảnh lên.",
        );
      });
  };
  ...
  const uploadProps: UploadProps = {
    beforeUpload: (file) => {
      ...
      if (!isPNGJPG) {
        message.error(
          `${file.name} không hợp lệ, vui lòng chọn ảnh đuôi .png/.jpg.`,
        );
      }
  ...
  const uploadTestProps: UploadProps = {
    beforeUpload: (file) => {
      ...
      if (!isXlsx) {
        message.error(
          `${file.name} không hợp lệ, vui lòng chọn file đuôi .xlsx.`,
        );
      }
    ...
    onChange: (info) => {
      const parseExcelToJson = (file: File) => {
        ...
        reader.onload = (e) => {
          ...
          jsonData[0] = jsonData[0].map((item) =>
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            const correctAnswerColumnIndex = jsonData[0].indexOf(
            ...
            <div className="flex flex-col  pl-4 ">
              <div className="flex-1 flex mb-4">
                <div className="flex-1 flex  mr-2">
                  ...
                  />
                  <Button loading={isLoading} onClick={handleLoadDocument}>
                    Tải trang
                  </Button>
                </div>
              </div>
              ...
              <div className="flex-1 flex mb-4">
                ...
                <Button
                  loading={isLoadingCheckImage}
                  onClick={handleCheckImageSaveAbility}
                >
                  Kiểm tra
                </Button>
              </div>
              ...
              <div className="flex-1 flex mb-4">
                <div className="flex-1 flex  mr-2">
                  ...
                  >
                    ...
                    <Select.Option value="LIFE_ENTERTAINMENT">
                      Đời sống - Giải trí
                    </Select.Option>
                    <Select.Option value="SCIENCE_TECHNOLOGY">
                      Khoa học - Công nghệ
                    </Select.Option>
                ...
                <div className="flex-1 flex  mr-2">
                  ...
                  >
                    ...
                    <Select.Option value="INFOGRAPHIC">
                      Infographic
                    </Select.Option>
