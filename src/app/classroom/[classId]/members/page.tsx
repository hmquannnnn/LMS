...
import {getAssigment, getClass, getStudentsList} from "@/apis/classAPI";
import {useDispatch, useSelector} from "react-redux";
import {getCurrentClassAction, getMembersAction} from "@/redux/slices/classSlice";
...
const ClassMember = (props: any) => {
    ...
    const membersList = useSelector(state => state.classes?.currentClass?.members || []);
    ...
    const getClassDetail = async() => {
        ...
        if(classInfo?.id) {
            ...
            const memberList = await getStudentsList(classInfo.id);
            const assignmentsList = await getAssigment(classInfo.id);
    ...
    return (
        ...
        <div className={"w-3/5 mx-auto"}>
                ...
                <Row>
                    ...
                    <Col span={19}>
                        {userRole === "ROLE_STUDENT" ? membersList.map((member: object, index: number) => (
                            <>
                                <p>{member.username}</p>
