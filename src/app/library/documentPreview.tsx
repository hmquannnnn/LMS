...
const DocumentPreview = ({ props }) => {
  ...
  const practiceItems: MenuProps["items"] = [
    {
      ...
      label: (
        ...
        >
          <span>{data?.type === "TEXT" ? "Đọc hiểu" : "Nghe và nói"}</span>
        </div>
      ),
    },
    {
  ...
  return (
    ...
    <div
      className={`w-full flex gap-[4vw]   ${props?.isOddIndex ? "flex-row-reverse" : "flex-row"}`}
    >
      ...
      <div className="mt-4 w-[55%] flex flex-col">
        <h1
          className={`text-[1.4em] font-timesNewRoman cursor-pointer  font-bold leading-[1.5em] mb-2 hover:text-purple_3 hover:duration-300 ${props.isOddIndex ? "text-right" : ""}`}
        >
          <Link scroll={true} href={`library/${document?.id}`}>
            {document?.title ? document?.title : props?.title}
          </Link>
        </h1>
        <p
          className={`text-[1em] font-[350] leading-[1.5em] line-clamp-3  ${props.isOddIndex ? "text-right" : ""}`}
        >
          {document?.veryFirstText
            ? document?.veryFirstText
            : props?.veryFirstText}
        </p>

        <div
          className={`w-full flex ${props.isOddIndex ? "justify-end" : "justify-start"}`}
        >
          <Link
            scroll={true}
            className="mt-5 mb-5 flex items-center px-5 hover:bg-opacity-80  py-0.5 rounded-3xl bg-purple_3 text-white font-josefin text-sm"
          ...
          >
            ...
            >
              <g transform="translate(-531 -17)">
                <path
                  d="M21.0113 9.08241C20.8996 8.97449 20.7216 8.97758 20.6137 9.08932 20.5084 9.19831 20.5084 9.37111 20.6137 9.48009L24.3475 13.214C24.3486 13.2151 24.3486 13.2169 24.3475 13.2179 24.347 13.2184 24.3463 13.2188 24.3456 13.2188L1.96875 13.2188C1.81342 13.2188 1.6875 13.3447 1.6875 13.5 1.6875 13.6553 1.81342 13.7812 1.96875 13.7812L24.3456 13.7812C24.3471 13.7813 24.3483 13.7825 24.3483 13.7841 24.3483 13.7848 24.348 13.7855 24.3475 13.786L20.6137 17.5199C20.5019 17.6278 20.4988 17.8059 20.6067 17.9176 20.7147 18.0293 20.8927 18.0324 21.0044 17.9245 21.0068 17.9222 21.0091 17.9199 21.0113 17.9176L25.2301 13.6988C25.3399 13.589 25.3399 13.411 25.2301 13.3012Z"
                  stroke="#FFFFFF"
                  stroke-width="0.315789"
                  fill="#FFFFFF"
                  transform="matrix(2.14815 0 0 1 531 17)"
                />
              </g>
            </svg>
          </Link>
        ...
        <div
          className={`flex flex-1 w-full gap-5 items-end  ${props.isOddIndex ? "justify-end" : "justify-start"}`}
        >
          <div
            className="max-w-[200px] h-min shadow-[rgba(0,0,0,0.15)_3px_3px_7px_1px] bg-purple_2 px-4 py-4 rounded-2xl cursor-pointer hover:shadow-[rgba(0,0,0,0.3)_3px_3px_7px_1px]"
            onClick={() => (isLiked ? onUnLike() : onLike())}
          >
            <div className="mb-2">
              {(!isLiked || isLiked == null) && <HeartLikeIcon />}
              {isLiked && <HeartUnLikeIcon />}
            </div>
            <div className="font-josefin font-bold text-white">Yêu thích</div>
            <div className="font-light text-xs">
              Thêm vào bộ sưu tập yêu thích để lưu lại những thông tin hữu ích!
            </div>
          </div>
          <Dropdown menu={{ items: practiceItems }} placement="bottom">
            <div className="max-w-[200px] h-min shadow-[rgba(0,0,0,0.15)_3px_3px_7px_1px] sha px-4 py-4 rounded-2xl cursor-pointer hover:shadow-[rgba(0,0,0,0.3)_3px_3px_7px_1px]">
              ...
              <div className="font-light text-xs">
                Thêm vào bộ sưu tập yêu thích để lưu lại những thông tin hữu
                ích!
              </div>
            </div>
          </Dropdown>
