    ...
    return (
        ...
        <div className={`w-full flex gap-[4vw] px-[8vw]  ${props.isOddIndex ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <div className='mt-4 w-[55%] flex flex-col'>
                <h1 className={`text-[1.4em] font-timesNewRoman cursor-pointer  font-bold leading-[1.5em] mb-2 hover:text-purple_3 hover:duration-300 ${props.isOddIndex ? 'text-right' : ''}`}><Link scroll={true} href={`library/${document.id}`}>{document?.title}</Link></h1>
