...
import {getAllClass} from "@/apis/classAPI";
    ...
    return (
        <>
            <Row className={"min-h-[78vh]"}>
                <Col md={4} className={"border-[1px] bg-gray-50 border-amber-500 px-5 py-3"}>
                    {userRole === "ROLE_TEACHER" ? <CreateClassButton onUpdate={handleUpdate}/> : <JoinClassButton onUpdate={handleUpdate}/>}
                </Col>
                <Col md={20}>
                    {classesList.map((classItem) => {
                        return (
                            <div className={"border-[1px] px-10 py-5"} key={classItem.id} onClick={() => router.push(`${paths.classroom}/${classItem.id}/notifications`)}>
                                <p>{classItem.name}</p>
                            </div>
