...
import LoggedInDropdown from "@/components/header/user/accountManagement/loggedIn";
import DefaultDropdown from "@/components/header/user/accountManagement/default";
    ...
    <>
      ...
      >
        <div className="w-1/6 flex items-center justify-center">
          <Link href={user?.role === ROLE_ADMIN ? paths.admin : paths.library}>
            <Image
              className={""}
              src="/images/logo.png"
              width={80}
                          ...
                          {listTopics.map((topic, index) => {
                            return (
                              <>
                                {index != 0 && (
                                  <div
                                    className={`h-full w-[1px] ml-2 mr-2 my-[2px] bg-black `}
                                  ></div>
                                )}
                                <Link
                                  key={index}
                                ...
                                >
                                  <div
                                    className={`text-lg text-nowrap items-center flex group${index} group${index}-hover:text-purple_1 h-[4vh]  hover:border-y border-purple_1  hover:`}
                                  >
                                    {topicMapping[topic]}
                                  </div>
                                </Link>
